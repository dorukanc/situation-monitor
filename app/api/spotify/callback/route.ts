...
export async function GET(request: NextRequest) {
  ...
  // Send tokens to opener via postMessage (works cross-origin: 127.0.0.1 → localhost)
  return new NextResponse(
    `<html><body><script>
      const data = {
      ...
      if (window.opener) {
        window.opener.postMessage({ type: "spotify-tokens", payload: data }, "*");
        window.close();
      } else {
        localStorage.setItem("sm-spotify-tokens", JSON.stringify(data));
